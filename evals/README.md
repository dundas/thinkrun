...
- Harness: `evals/run-eval.ts` (bun). `build` assembles a routing prompt from the live `SKILL.md` frontmatter descriptions (read at build time, never copied) + all fixtures; `grade` scores strict JSON output. `build-variant`/`grade-variant` run catalog ablations that test whether sibling skills absorb a removed skill's traffic.
